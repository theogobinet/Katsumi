...
def display():
    ...
    print("")
    print("-- Kasumi --")
    print(("Kasumi per bloc time: {:.3f} milliseconds").format(
        config.WATCH_BLOC_KASUMI * 1000))
    print(("Kasumi global time: {:.3f} seconds").format(
        config.WATCH_GLOBAL_KASUMI))
    print(("Kasumi's total runtime: {:.2f}%").format(
        (config.WATCH_GLOBAL_KASUMI /
         (time.time() - config.WATCH_GLOBAL_TIME)) * 100))
    print("")
    print(("-- CIPHER: {} --").format(config.WATCH_CIPHER_TYPE))
    print(("{} per bloc time: {:.3f} milliseconds").format(
        config.WATCH_CIPHER_TYPE,
        (config.WATCH_BLOC_CIPHER - config.WATCH_BLOC_KASUMI) * 1000))
    print(("{} global time: {:.3f} seconds").format(
        config.WATCH_CIPHER_TYPE,
        config.WATCH_GLOBAL_CIPHER - config.WATCH_GLOBAL_KASUMI))
    print("")
    if config.GALOIS_WATCH:
        print("-- GALOIS INVERSION --")
        print(("Inversion number: {:.0f} ").format(
            config.WATCH_INVERSION_NUMBER))
        print(("Inversion per Kasumi: {:.0f} ").format(
            config.WATCH_INVERSION_NUMBER / config.WATCH_KASUMI_NUMBER))
        print(("Inversion global time: {:.3f} seconds").format(
            config.WATCH_GLOBAL_INVERSION))
        print(("Inversion's total runtime: {:.2f}%").format(
            (config.WATCH_GLOBAL_INVERSION /
             (time.time() - config.WATCH_GLOBAL_TIME)) * 100))
        print(("Modular mult per inversion: {:.0f} ").format(
            config.WATCH_MULT_NUMBER / config.WATCH_INVERSION_NUMBER))
        print(("Modular mult time: {:.3f} seconds").format(
            config.WATCH_GLOBAL_MULT))
        print(("Modular mult's total runtime: {:.2f}%").format(
            (config.WATCH_GLOBAL_MULT /
             (time.time() - config.WATCH_GLOBAL_TIME)) * 100))
        print("")
    print("-- Stats --")
    ...
    print(("Time left: {:.3f} seconds"
           ).format(((100 / config.WATCH_PERCENTAGE) *
                     (time.time() - config.WATCH_GLOBAL_TIME)) -
                    (time.time() - config.WATCH_GLOBAL_TIME)))
